...
!!! tip ""
    ...
    - A microSD card with free space of least 32 gb

### Instructions

!!! tip ""

	1. Copy the `bootloader` folder from the Hekate `.zip` file to the root of your SD card
    2. Copy `Lockpick_RCM.bin` to the `/bootloader/payloads` folder on your SD card	
    3. Enter RCM and inject the Hekate payload
    4. Use the touch screen to navigate to `Tools` and then `Backup eMMC`
    5. Tap on `eMMC BOOT0 & BOOT1`
       - This may take a few minutes
    6. Tap on `Close` to continue, then tap on `eMMC RAW GPP`
    ...
    7. Press `Close` > `Home` > `Power Off`
    8. Insert your SD card into your PC
    9. Copy the `backup` folder on your SD card to a safe location on your PC
